...
public enum TokenType {
    ...
    PING(TokenCategory.SENSOR, "ping"),
    ...
    /**
     * Returns the integer value extracted from mem's syntactic sugar
     * @param memsugar must be of TokenCategory MEMSUGAR
     * @return the integer value of the syntactic sugar, or -1 if the memsugar wasn't found
     */
    public static int memsugarToValue(TokenType memsugar){
        assert memsugar.category.equals(TokenCategory.MEMSUGAR);
        for(int i=39;i<=45;i++){
            if(values()[i].equals(memsugar)) return i-39;
        }
        throw new IllegalArgumentException("memsugar does not exist");
    }
