...
# ╔═╡ e82fbc2e-dd32-4803-bed0-8a01410f19dc
using Test
...
# ╔═╡ ac711eca-e963-4142-aecd-d6a4ac19abbb
md"""
To intuitively understand what this solution is doing, both functions will take the $$O(n^3)$$ time to factorize $$\Phi_{XX}$$. 

The first function computes $$cs = \Phi_{XX}^{-1}\theta_X(t)$$, which will take $$O(n^2m)$$ because it will have to do the solving process (which is $$O(n^2)$$ for each column vector, of which $$\theta_X(t)$$ will have $$m$$ column vectors), and then matrix multiply for $$cs^{T}w$$, which will take $$O(mn)$$ time, which when you add it all up together, will result in $$O(n^3)+O(n^2m)$$

My (second) function takes advantage of the fact that $$\Phi_{XX}$$ is symmetric, so we know that $$\Phi_{XX}^{-T} = \Phi_{XX}^{-1}$$. So, to write an equivalent expression, we can do $$\theta_X(t) ^ T v$$, where $$v=\Phi_{XX}^{-1}w$$, where the former expression is obviously $$O(mn)$$ by matrix multiplciation, and the latter expression must be $$O(n^2)$$, which by Big O Notation, is $O(n^3) + O(mn)$
"""
...
# ╔═╡ 2751ea06-05e9-4d3b-acf8-e6e1e61f1167
test_sensor_demo(2001,5000)

# ╔═╡ 11922f25-8fb6-4625-aa1e-3b3abfdc87a6
md""" 
Since the norm is so small, it seems like we do get the same results, and it seems like my code takes roughly $$0.180$$ the time, which somewhat reflects the Big O discrepancies, as here, $$n=2001$$ and $$m=5000$$, the ratio for the first function in the denominator and mine on the numerator should roughly be $$\frac{2001^3+2001*5000}{2001^3 + 2001^2*5000}$$, which is around 0.286, so it seems within the ballpark
"""
...
function fill_missing!(F, fX, k)
	# Use block elimination to find that r_k = ((e_k)^T ϕ^-1 (f tilde X)) / ((e_k)^T ϕ^-1 (e_k))
	# This is f tilde X
	fX[k] = 0.0
	#ϕ^-1 * f tilde X
	v = F \ fX
	...
	# ϕ^-1 * e_k
    w = F \ e
	#instead of doing (e_k)^T, realize that it just takes the kth element of each vector of ϕ^-1 * f tilde X or ϕ^-1 * e_k
	rk = v[k] / w[k]
    fX[k] = -rk
    return fX[k]
end

# ╔═╡ 7d469f96-d7c4-4ce4-8a28-95e8330186c3
md""" 
Theoretically, this should only take $$O(n^2)$$, because the most expensive operations done here is the Inverse Solving, which only takes $$O(n^2)$$ by definition of the Cholesky Factorization.
"""

# ╔═╡ 1c22f9ce-9a59-4a21-961d-e2cc363998c5
begin
	...
	l = 1/n
    φ(r) = exp(-(r/l)^2)
    Φ = [φ(abs(x[i] - x[j])) for i in 1:n, j in 1:n]
    F = cholesky(Φ)
    ...
    for k in 1:n
        fX = copy(f_true)
        imputed = fill_missing!(F, fX, k)

		mask = collect(1:n) .!= k
		
		Φ_reduced = Φ[mask, mask]
		f_reduced = f_true[mask]
		c = Φ_reduced \ f_reduced
		f_est_true = dot(Φ[k, mask], c)

        # Test correctness
		@test isapprox(imputed, f_est_true, atol=1e-10)
		println("imputed = $imputed, true = $f_est_true, difference = $(imputed - f_est_true)")
    end
end
...
# ╔═╡ 78613e8c-200c-4051-9903-ace5aa8e8337
md"""
We know that from Linear Algebra, that the determinant of this matrix, let's call it $$A$$, is the same as the determinant of its Upper Triangular Form (which is the product of the diagonals), $$U$$. 

Use Block Elimination to do so, and you will find that the matrix $$U$$ will be $$\begin{bmatrix} \Phi_{XX} & \Phi_{Xz} \\ 0 & \phi(0) - \Phi_{zX} \Phi_{XX}^{-1} \Phi_{Xz} \end{bmatrix}$$

So, $$\det \begin{bmatrix} \Phi_{XX} & \Phi_{Xz} \\ 0 & \phi(0) - \Phi_{zX} \Phi_{XX}^{-1} \Phi_{Xz} \end{bmatrix}$$ is $$(\Phi_{XX}) \times (\phi(0) - \Phi_{zX} \Phi_{XX}^{-1} \Phi_{Xz})$$

Since we can only modify with respect to placement of $$z$$, that means we can only maximize $$\phi(0) - \Phi_{zX} \Phi_{XX}^{-1} \Phi_{Xz}$$ (which is the Schur Complement)
""" 
...
function greedy_select(ϕ, X, k)
	...
	for j = 1:k
		...
		# swaps previously built cols of R
        for i = 1:j-1
...
# ╔═╡ a548d044-43d3-41be-9861-209fa0b3e601
begin
    using Random
    rng = Random.Xoshiro(120)
    h = 1.0
    phi = scale_rbf(ϕ_se, h)

    for N in [5, 10, 20, 40]
        for k in 1:min(5, N)
            X = rand(rng, 2, N)
            p_naive, R12_naive = naive_greedy_select(phi, X, k)
            p_fast,  R12_fast  = greedy_select(phi, X, k)
            @test p_fast[1:k] == p_naive[1:k]
			println(p_fast[1:k], p_naive[1:k])
			println(X[:, p_fast[1:k]], X[:, p_naive[1:k]])
        end
    end
end
...
# ╔═╡ 94a04c12-b008-4601-91ab-cb125690cedd
hello_test2d_se()

# ╔═╡ 6a713c95-32fc-45d8-98a2-753911c5123c
hello_test2d_se(σ=3.0)

# ╔═╡ 65eb2be8-5099-4bb1-abd9-266a622415b9
hello_test2d_se(l=.05)

# ╔═╡ 0ab3d2b9-032a-4386-a630-c6ba3ecf6372
hello_test2d_se(σ=3.0,l=.05)

# ╔═╡ 33550301-f4c3-4397-953a-caa3d6027b8d
md""" 
When we set $l = 0.05$ or $\sigma = 3$, it will return an exception stating that the given matrix is not positive definite, so we can't do Cholesky Factorization. This is because of the hard truncation caused by the kernel, and when the cutoff removes significant entries when they are far enough, this can break the positive definite nature of $$Φ_{XX}$$
"""
...
# ╔═╡ e3f5adf4-0fd0-486d-9f67-3f10edf49fbb
md""" 
We know by definition that $$|\hat{s}(x)-s(x)| = |\hat{\Phi}_{xX} c_0 - \Phi_{xX} c|$$, if we redo the addition, we know that $$|\hat{s}(x)-s(x)| = |(\hat{\Phi}_{xX} -\Phi_{xX})c_0 + \Phi_{xX}(c_0 -  c)|$$, which by the triangle inequality, we know that it is $$\leq |(\hat{\Phi}_{xX} -\Phi_{xX})c_0| + |\Phi_{xX}(c_0 - c)|$$

By using the fact that $|u \cdot v| \leq \|u\|_1 \|v\|_\infty$ for any $u$ and $v$, we know that $$|(\hat{\Phi}_{xX} -\Phi_{xX})c_0| \leq ||\hat{\Phi}_{xX} -\Phi_{xX}||_{\infty} ||c_0||_1$$ and $$|\Phi_{xX}(c_0 - c)| \leq ||\Phi_{xX}||_{\infty} ||c_0 - c||_1$$

To bound $\|\Phi_{xX}\|_\infty$, notice that $$\phi(r) = e^{-r^2/l^2}$$, and by definition $$r \geq 0$$, so by the definition of the exponential function, $$\phi(r) \leq 1$$, so for the max norm of the vector, $\|\Phi_{xX}\|_\infty \leq 1$

To bound $\|\hat{\Phi}_{xX}-\Phi_{xX}\|_\infty$, notice that by definition of the cutoff approximation,

- if $\|x-x_i\| \le r_{\mathrm{cutoff}} = \sigma l$, then
$$\hat{\Phi}_{xX,i} = \Phi_{xX,i}$$

- if $\|x-x_i\| > \sigma l$, then
$$\hat{\Phi}_{xX,i} = 0$$
and
$$\Phi_{xX,i} = e^{-\|x-x_i\|^2/l^2} \le e^{-\sigma^2}.$$

Therefore,
$$|\hat{\Phi}_{xX,i} - \Phi_{xX,i}| \le e^{-\sigma^2}.$$

Taking the maximum over all entries gives
$$\|\hat{\Phi}_{xX}-\Phi_{xX}\|_\infty \le e^{-\sigma^2}.$$

Putting everything together, we obtain
$$|\hat{s}(x)-s(x)|
\le \|c_0\|_1 e^{-\sigma^2} + \|c-c_0\|_1.$$
"""
...
function hello_test2d_se_itref(npoints=20000; σ=4.0, σ1=6.0, l=0.01, η=0.0, 
	...
	if verbose
		...
		F = cholesky(ΦXX1)
		c = F\fX
		d(z) = sum(ϕ(norm(z-xi))*ci for (xi,ci) in zip(eachcol(xy),c))
		time_d = time() - time_d
	end
	...
	# Look at density of ΦXX and error at center point
	if verbose
		rerr_d = (ftest([0.5; 0.5])-d([0.5; 0.5]))/ftest([0.5;0.5])
		println("direct method:")
		r_d=nnz(ΦXX1)/npoints^2
		println("  nonzero elements ratio: $r_d; \n  relative error: $rerr_d;\n  time: $time_d")
		println("========================")
	end
	r_i=nnz(ΦXX0)/npoints^2
	rerr_i = (ftest([0.5; 0.5])-s([0.5; 0.5]))/ftest([0.5;0.5])
	println("iterative method:")
	println("  nonzero elements ratio: $r_i; \n  relative error: $rerr_i;\n  time: $time_i")	
end

# ╔═╡ 0b627ccc-5eef-4852-8d5e-1b4b089ae427
hello_test2d_se_itref()

# ╔═╡ 2aaa1dc2-3343-472d-8d34-99331ed507ca
md"""
As can be seen from the above running results, while both the direct and the iterative methods return similar results with the same relative errors, the direct method used 0.5429 sec, while the iterative one only used 0.233 sec; furthermore, the direct method's matrix has 0.01 non-zero elements, while the iterative one has only 0.00047 non-zero elements, far sparser. Thus the iterative method reduced the time and space complexity. This is because iterative refinement will shift work from expensive factorization of the larger 6l matrix to the 4l matrix and replace it with relatively cheap matrix vector multiplications, and the relative errors are similar. 
"""

# ╔═╡ 58d35e9a-6608-4ab7-85e2-21b49f3a1f43
md"""
I think we use $$6l$$ as our "true" problem because for the squared exponential kernel, $$\phi(6l) = e^{-\frac{(6l)^2}{l^2}} = e^{-36} \approx 2*10^{-16}$$, which is approximately the machine epsilon/precision for Julia's Float64, so we aren't going to truncate any meaningful interactions beyond that (negligible numerically)
"""

# ╔═╡ a9f21e72-897e-4df3-8d56-84d16c00ab70
hello_test2d_se(l=0.03)

# ╔═╡ 17b10755-03c2-4eba-b435-50b868210c24
hello_test2d_se_itref(l=0.02, verbose=false)

# ╔═╡ bf5bc899-d61b-4d11-b24c-0e69f0b94bf9
hello_test2d_se_itref(l=0.02, η=0.1)

# ╔═╡ 0a8f7496-96c7-4d67-a02f-99d6729a3cac
hello_test2d_se_itref(σ=2.0, verbose=false)

# ╔═╡ f40ad638-b8ed-42cc-b868-9519dbc13859
hello_test2d_se_itref(σ=2.0, η=0.1, verbose=false)

# ╔═╡ 8940d884-3c15-407f-8cdc-fc31c5858ea8
md"""
As seen the foregoing 4 cells, when $\eta=0.0$, the function returns exceptions: "the matrix is not positive definite" when $l$ is larger, say $l=0.02$ or $\sigma$ is smaller, e.g., $\sigma=2.0$. By setting $\eta=0.1$, both exceptions are gone, the function can work without problem, because adding $$\eta I$$ for some large enough $$\eta$$ will ensure positive definiteness by shifting all the eigenvalues up to some number above 0
"""
...
manifest_format = "2.0"
project_hash = "8d1d2b9c499c140ca2ef1fd887ca11a8877b66ed"
