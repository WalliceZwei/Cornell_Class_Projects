...
# ╔═╡ a5f5bef5-1c09-4e6b-b68a-dec857319b67
deblur_img(blurry_rgb, s, 0)

# ╔═╡ fab21b7f-266d-4d20-90e4-61d34864230f
deblur_img(blurry_rgb, s, 0.01)

# ╔═╡ bd489526-f294-4905-ba57-eab5363289d1
md"""
1. This naive deblurring approach with $\lambda = 0$ doesn't generate the original image, but when we set $\lambda = 0.01$ which does some regularization, it approximately does generate the original image (look down below for the reason, small singular values can cause large amplification in rounding error $$E$$). 
""" 

# ╔═╡ b4950fc6-67e6-4ae5-832f-a32644a91d59
md"""
2. 

Using the fact that $H$ is diagonalizable and that $$Z^* = Z^{-1}$$, then the solution to the Tikhonov would be $$(H^*H+\lambda ^ 2 I) V_{orig}= H^*V_{blur}$$. If we expand this out with the eigendecomposition, we get $((Z^{-1}SZ)^* (Z^{-1}SZ) + Z^{-1}\lambda ^ 2 IZ)V_{orig} = (Z^{-1}SZ)^* V_{blur}$. 

Use the fact that $$(Z^{-1}SZ)^* = Z^{-1}S^*Z$$ and that $S^*S = |S|^2$ by some basic complex math to simplify and get $$Z^{-1}(|S|^2 + \lambda ^2 I) Z V_{orig}= Z^{-1}S^*ZV_{blur}$$

Apply $$Z$$ to both sides to get $$(|S|^2 + \lambda ^2 I) Z V_{orig}= S^*ZV_{blur}$$

So, $$V_{orig}= Z^{*}(|S|^2 + \lambda ^2 I)^{-1}S^*ZV_{blur}$$

The function $$deblur$$ does these exact steps, as it will first call $$img\_fft(V_{blur})$$, which will calculate $$ZV_{blur}$$, then $$blur\_fft$$ will use $$deblur\_factors$$ to apply $$(|S|^2 + \lambda ^2 I)^{-1}S^{*}$$ elementwise for the diagonal, and then $$img\_ifft$$ will apply $$Z^{*}$$ to finish it off and go back to real space, which matches the Tikhonov Solution derived. 
""" 

# ╔═╡ 71c199cf-5a50-4b1f-9d75-dc7471c5fb76
md"""
3. The 8-bit values are rounded to the nearest integer in $$0...255$$ which is then normalized to $$[0,1]$$. The max error is then $$|E_{ijk}| \leq \frac{1}{2*255}$$ for each color channel $$k$$ for this rounding, so the Frobenius Norm is 

$$\|E\|_F^2 = \sum_{i,j,k}(E_{ijk})^2 \leq \sum_{i,j,k}(\frac{1}{2*255})^2$$

Obviously, this can be rewritten, there are $$i*j*k$$ summations of $$(\frac{1}{2*255})^2$$, and we can rewrite $$i*j*k$$ as $$3N_{pixels}$$, where $$i*j$$ is represented by $$N_{pixels}$$, and there are three color channels ($k$)

So, $$\|E\|_F^2 \leq 3N_{pixels}(\frac{1}{510})^2$$
"""

# ╔═╡ 7665bffb-ff1b-4a86-bf04-eff338792fd2
md""" 
4. 
The singular values of $$H$$ are the absolute value of its eigenvalues, so $$\sigma_{k} = |s_{k}|$$. This is because to get the singular values, we find the square root of the eigenvalues of $$H^{*}H$$, which we know from above that $$H*H$$ is equal to (note that $$|S|^2 = S^2$$) $$Z^*|S|^2Z$$, so the square root of the eigenvalues will be $$|s_k|$$

The smallest singular value will be $$\sigma_{min} = min_{k}|s_k|$$ 
"""

# ╔═╡ ccacb578-1e23-4efe-975c-8311bb30fd24
md"""
5. To find $\|V^{\mathrm{orig}}-V^{\mathrm{naive}}\|_F$ which is obviously equivalent to $\|V^{\mathrm{naive}}-V^{\mathrm{orig}}\|_F$, realize that $$V^{naive} = H^{-1}V^{blur}$$, and since $$V^{blur} = HV^{orig} + E$$, we can substitute to know that $$V^{naive} = H^{-1}(HV^{orig} + E) = V^{orig}+H^{-1}E$$, so $\|V^{\mathrm{orig}}-V^{\mathrm{naive}}\|_F = ||V^{orig} - (V^{orig}+H^{-1}E)||_F = ||H^{-1}E||_F \leq ||H^{-1}||_2 ||E||_F$

We know that we can use the two norm for $$||H^{-1}||_2 = \frac{1}{\sigma_{min}(H)}$$ and since $$||E||_{F}^{2} \leq \frac{3N_{pixels}}{510^2}$$, we know that $$||E||_{F} \leq \frac{\sqrt{3N_{pixels}}}{510}$$, so 

$$\|V^{\mathrm{orig}}-V^{\mathrm{naive}}\|_F=  \frac{||E||_F}{\sigma_{min}(H)} \leq \frac {\sqrt{3N_{pixels}}}{510\sigma_{min}(H)} $$

We know that $$||V^{blur}||_F = \sqrt{3N_{pixels}}$$

We know that for $$H$$, $$\sigma_{min}(H)$$ can get very small, and when this is the case, $$\frac{1}{{510\sigma_{min}(H)}}$$ will get huge, maybe even above 1 or very large to say the least, which will mean that the reconstruction error ($$\|V^{\mathrm{orig}}-V^{\mathrm{naive}}\|_F$$) is larger than the image signal itself (represented by $$||V^{blur}||_F$$)
...
function discrepancy(V, s, λmin=1e-4, λmax=1)
	tau = τ_morozov(V)
    f(λ) = resid_norm(V, s, λ) - tau
    return fzero(f, λmin, λmax)
end
...
begin
	...
	λ_opt = discrepancy(blurry_rgb, s)
    ...
    plot(λs, ρs,
        ...
        label="residual norm",
        linewidth=2)
    hline!([τ], linestyle=:dash, label="τ (Morozov threshold)")
    vline!([λ_opt], linestyle=:dot, label="λ* (discrepancy)")
    scatter!([λ_opt], [τ], markersize=8, label="operating point")
	
end

# ╔═╡ 8a0788f0-8026-4d0d-a1b4-fef9ea16f13b
md""" 
This value seems to be correct, because $$\lambda$$ seems to fall near where the "knee" of the $$p(\lambda)$$ curve is. This structure is correct because before the knee, the images are very noisy since they try to chase being better reconstructions of $$V^{blur}$$ with the $$p(\lambda)$$ objective which will create a noisier reconstruction (the problem we mentioned in 5) and more deblurring, but after the knee, we do less deblurring, so reblurring will lose a lot of the original input $$V^{blur}$$, which will cause $$p(\lambda)$$ to blow up. 
""" 
...
# ╔═╡ f79b096b-7443-4fe4-9887-b3eff4a28885
md"""
1. Using matrix calculus, we know that $\frac{d}{d\lambda} \left( I-H \hat{H}^\dagger(\lambda) \right)$ is equivalent to $\frac{d}{d\lambda} ( I-H (H^*H + \lambda^2I)^{-1}H^*)$, and we only care about the terms that depend on $\lambda$ and everything else besides $\lambda$ can be thought of as a constant, so this is equivalent to $-H \frac{d}{d\lambda}(H^*H + \lambda^2I)^{-1} H^*$, which we can evaluate (knowing the matrix inverse derivative being $\frac{d}{d\lambda}A^{-1} = -A^{-1}\frac{dA}{d\lambda}A^{-1}$ and knowing that $\frac{d}{d\lambda}(H^*H + \lambda^2I)$ is $2\lambda I$), we can evaluate to a final expression of $2\lambda H(H^*H + \lambda^2I)^{-2}H^*$

We can rephrase this as $\hat{H}^\dagger(\lambda)^* = H (H^*H + \lambda^2I)^{-T}$ and $\hat{H}^\dagger(\lambda) = (H^*H + \lambda^2I)^{-1}H^*$, and since $(H^*H + \lambda^2I)^* = H^*H + \lambda^2I$ (Hermetian because $(A+B)^* = A^*+B^*$), we can write it as $2\lambda H(H^*H + \lambda^2I)^{-2}H^*$ $=$ $2\lambda( \hat{H}^\dagger(\lambda) )^* \hat{H}^\dagger(\lambda)$
""" 
...
function gcv(V, s, λ, Z)
    ...
    N = length(V)
    m = size(Z,1)
    ρ2 = resid_norm(V, s, λ)^2

    dρ2 = 4*λ * real(dot(vec(deblur(V, s, λ)), vec(deblur(resid(V, s, λ), s, λ))))

    S = 0.0
    dS = 0.0
	
    for l in 1:m
        z = repeat(Z[l:l,:,:], 3, 1, 1)
        Az = z - blur(deblur(z, s, λ), s)
        S += real(dot(z[:], Az[:]))
        Hz = deblur(z, s, λ)
        dS += 2*λ * norm(Hz)^2
    end
...
function gcvopt(V, s, λmin, λmax, Z)
    return fzero(λ -> gcv(V, s, λ, Z)[2], λmin, λmax)
end
...
begin
	...
	plot(λs, Gs, xscale=:log10, yscale=:log10, label="G̃(λ)", xlabel="λ", ylabel="G̃")
    scatter!([λ_gcv], [Gλ_gcv], label="λ_gcv", marker=:star, markersize=8)
end

# ╔═╡ bf0beceb-b7f0-479f-9144-cd31383cca16
md""" 
With the log-log plot of $\tilde{G}(\lambda)$, we see that near the optimal $\lambda$, the curve looks very flat and looks like it is a minimum, as small pertubings of $\lambda$ don't change the value of the criterion that much, which means that this solution is somewhat stable. (It doesn't look too spikey, it looks flat in the neighborhood).
""" 
...
function lcurveopt(blurry_rgb, s)
    λs = exp10.(range(-5, 3, length=1500)) 
    return λs[argmax(lcurve(blurry_rgb, s, λ)[3] for λ in λs)]
end

# ╔═╡ 03f6ab2f-a40c-4c75-9dd3-7c85f53835b1
print(lcurveopt(blurry_rgb, s))

# ╔═╡ 6280028a-fa84-478a-af70-13953698f32b
function plot_lcurve(blurry_rgb, s, τ)

    λ_lcurve = lcurveopt(blurry_rgb, s)
	λs = exp10.(range(-8, 3, length=1500))

	ρs = [lcurve(blurry_rgb, s, λ)[1] for λ in λs]
    ηs = [lcurve(blurry_rgb, s, λ)[2] for λ in λs]
    κs = [lcurve(blurry_rgb, s, λ)[3] for λ in λs]

    ρ_lcurve, η_lcurve, κ_lcurve = lcurve(blurry_rgb, s, λ_lcurve)
	# TODO: Produce the plot
    p1 = plot(ρs, ηs, xscale=:log10, yscale=:log10,
              xlabel="ρ (residual norm)", ylabel="η (solution norm)",
              label="L-curve")
    scatter!(p1, [ρ_lcurve], [η_lcurve], label="λ* (L-curve)",
             marker=:star, markersize=8, color=:red)

	# TODO: Produce the curvature
    p2 = plot(ρs, κs, xscale=:log10,
              xlabel="ρ (residual norm)", ylabel="κ (curvature)",
              label="curvature")
    scatter!(p2, [ρ_lcurve], [κ_lcurve], label="λ* (L-curve)",
             marker=:star, markersize=8, color=:red)
...
p1lc, p2lc, λ_lcurve = plot_lcurve(blurry_rgb, s, τ)
...
# ╔═╡ 0cf020b5-ce5a-4c8e-9668-1f8eeebc12d5
md"""
1. Yes, there is a clear corner with a well defined curvature peak. 
"""
...
# ╔═╡ c745bea0-2452-4b31-84c0-ffa1e655b76a
md"""
2. This is the image for a very small lambda, roughly $$1e-4$$, which results in a very noisy image when deblurred, this is not a good result. 
"""
...
manifest_format = "2.0"
project_hash = "17095a7ce416b7c208a6fa49c5f96b84f7daa1e4"
