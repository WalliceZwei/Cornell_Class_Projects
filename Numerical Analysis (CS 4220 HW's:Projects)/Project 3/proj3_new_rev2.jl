...
Give an example $A, B$ where componentwise steepest ascent must nonetheless
diverge for any positive $\alpha$.
"""

# ╔═╡ b5001352-85d7-4099-8686-fde7a1c76530
md"""

We know that $\frac{\partial f_1}{\partial x_1}(x) = -2(A_{11}x_1 + A_{12}x_2)$ and that $\frac{\partial f_2}{\partial x_2}(x) = -2(B_{21}x_1 + B_{22}x_2)$

So, 

$$x_1^{k+1} = x_1^k - 2\alpha (A_{11}x_1^{k} + A_{12}x_2^{k})$$

$$x_2^{k+1} = x_2^k - 2 \alpha(B_{21}x_1^{k} + B_{22}x_2^{k})$$

We can rewrite this as 

$$x^{k+1} = \begin{pmatrix} x_1^{k+1} \\ x_2^{k+1}  \end{pmatrix} = \begin{pmatrix} 1 - 2\alpha A_{11} & - 2\alpha A_{12} \\ - 2\alpha B_{21} & 1 - 2\alpha B_{22} \end{pmatrix}\begin{pmatrix} x_1^{k} \\ x_2^{k}  \end{pmatrix}=(I-2\alpha C)\begin{pmatrix} x_1^{k} \\ x_2^{k}  \end{pmatrix}$$

where $$C=\begin{pmatrix} A_{11} & A_{12}\\ B_{21} & B_{22}\end{pmatrix}$$

We know that it will diverge if the spectral radius is above 1 for all positive $\alpha$ for $D(\alpha) = I-2\alpha C$.  Since the eigenvalues of $D(\alpha)$ are
$\mu_i = 1-2\alpha \lambda_i$, where $\lambda_i$'s are the eigenvalues of matrix $C$. Because $\alpha$ is positive, if one $\lambda_i$ is negative, then $|\mu_i| > 1$

An easy way to do this is to see that if we can find two positive definite matrixes A and B so that the matrix C based on the first and second row of A and B can have negative eigenvalues. 

So, if $$A = \begin{pmatrix} 1 & 3 \\ 3 & 10  \end{pmatrix}$$ and 
$$B = \begin{pmatrix} 1 & 2 \\ 2 & 5  \end{pmatrix}$$, then A is positive definite since $A_{11}=1>0$, and $detA=10-9=1>0$; B is also positive definite since $B_{11}=1>0$, and
$detB=5-4=1>0$.

Plugging A and B to formulate C, we have $$C=\begin{pmatrix} 1 & 3 \\ 2 & 5  \end{pmatrix}$$, whose 2 eigenvalues $\lambda_i$'s are 6.1623 and -0.1623, the negative $\lambda_-$ making one $\mu_-=1+2*0.1623 = 1.3246 > 1$,
then we know that the iteration matrix has a spectral radius (smallest eigenvalue) larger than 1, this must diverge. 
"""
...
# ╔═╡ 10a73dd0-58c2-42f4-88ba-72e23c1d326d
md"""

For the Even player, if we know that $y$ is fixed and at Nash Equilibrium, then maximizing $$x^TAy$$ subject to the basic weight/probability restraints is equivalent to 

$$max_{x}\sum_{i} x_i (Ay^*)_i$$

Because we know this is a mixed strategy, if there were one coordinate (for example $(Ay^*)_k$ for some index $k$), then the player Even would place all of its chips ($x_k$) into that one ($x_k = 1$), which would no longer make it a mixed strategy, but a pure strategy. 

Therefor, for a mixed strategy (multiple $x_i > 0$), we know that

$$(Ay^*)_1 = \ . . . \ = (Ay^*)_n = \alpha$$

So by vector operations, $$Ay^*=\alpha e$$

"""

# ╔═╡ d0fca119-d83d-4d69-804c-6f3a9935c4f4
md"""
For the Odd player, if we know that $x$ is fixed and at Nash Equilibrium, then maximizing $$-x^TAy$$ subject to the basic weight/probability restraints is equivalent to 

$$max_{x}\sum_{i} y_i (-A^Tx^*)_i$$

Because we know this is a mixed strategy, if there were one coordinate (for example $(-A^Tx^*)_k$ for some index $k$), then the player Odd would place all of its chips ($y_k$) into that one ($y_k = 1$), which would no longer make it a mixed strategy, but a pure strategy. 

Therefore, for a mixed strategy (multiple $x_i > 0$), we know that

$$(-A^Tx^*)_1 = \ . . . \ = (-A^Tx^*)_n = -\beta$$

So by vector operations and cancelling the negative signs on both sides, $$A^Tx^*=\beta e$$
""" 
...
# ╔═╡ 2ed8a7cc-6ff7-4560-983e-9e24a95aa7d6
function Jreplicator(x, A)
	σ = dot(x, A, x)
    Diagonal(A*x .- σ) + Diagonal(x)*(A .- ((A+A')*x)')
end
...
# ╔═╡ ddf56197-5e34-44f7-91d1-ce61a7d18e56
md"""
By expanding the matrix-vector product, this matrix vector expression is better simplified as the proof of 1) $Ax^* = \bar{f}e$, and 2) $e^T x^*=1$, in vector form.

Since we know that the Nash Equilibrium is when $F(x^*) = 0$, or when 

$$\forall i, 0 = x^*_i ((Ax^*)_i - (x^*)^T A x^*) \rightarrow (Ax^*)_i = (x^*)^T A x^*$$

And since we know by definition that

$$\bar{f} = \bar{f}(x^*) = (x^*)^T A x^*$$

So, $(Ax^*)_i = \bar{f}$, and since this applies for all $i$, we can vectorize it into:

$$Ax^* = \bar{f}e   \tag{1}$$

As the replicator dynamics are defined on the simplex with constraint $e^T x = 1$, the stationary point $x^*$ by definition of the state space must satisfies:

$$e^T x^* = 1. \tag{2}$$

Combining the above 2 proved equations, we thus proved the matrix equation.

Now prove there is at most one equilibrium by contradiction: Assume there are at least 2 different interior equilibra $x^*$, $y^*$ with corresponding $\bar{f_x}$ and $\bar{f_y}, then we have

$$Ax^* = \bar{f_x} e, e^Tx^*=1,$$

$$Ay^* = \bar{f_y} e, e^Ty^*=1,$$

Taking the differences of the above equations:

$$Az = b e, e^Tz = 0, \tag{3}$$

where $z=x^*-y^*$ which is not 0 by assumption, and $b=\bar{f_x}-bar{f_y}$.

Now consider the cases for $b$:

1) if $b \not= 0$, then by (3) $e=Az/b$, which belongs to Range(A), contradicting the assumption $e\not\in Range(A)$

2) if $b=0$, the (3) turns into:

$$Az=0, e^Tz=0 \tag{4}$$

by assumption, $z\not=0$ and $z \in ker A$. As we knew $dim(ker A) \leq 1$, any nonzero vector in ker A spans the null space.

we know that 

$$Range(A^T)=(ker A)^\perp$$

and given that $e\not\in Range(A^T)$, so e is not orthogonal to kerA, thus, for any nonzeor $a \in ker A$, $e^T a \not= 0$, but we knew $z \in Ker A$ and z is nonzero, we must have $e^Tz \not= 0$, contradicting $e^Tz=0$ in (4).
so case 2 also causes contradiction.

We thus proved that it is impossible to have two or more equilibra, therefore we can have at most one equilibium, which concludes the proof of this question.
...
# ╔═╡ 49e288db-df17-4285-85d4-75a470462833
function rps_period_refine(x0, T0)
	max_iter=100
	tol=1e-12
	T_sol = T0
	grad_norms = Float64[]
	...
	for k in 1:max_iter
		xT = solve_replicator(Arps, x0, T_sol)[end]
		r = xT-x0 
		Ax = Arps * xT
		J = Ax .* xT .- dot(xT, Ax) .* xT
		g = J' * r  
        push!(grad_norms, abs(g))
        if abs(g) < tol
	...
	return T_sol, grad_norms
end

# ╔═╡ 2e09a4f0-25a7-44b4-a398-b53e9d0c62c5
begin
	
    T_refined, grad_normsy = rps_period_refine([0.1; 0.2; 0.7],
                                rps_period_est([0.1; 0.2; 0.7], 20.0))

	println(T_refined)
	
    p = plot(0:length(grad_normsy)-1, grad_normsy,
        yscale  = :log10,
        color   = :crimson,
        marker  = :circle,
        xlabel  = "iteration k",
        ylabel  = "‖Jᵀr‖",
        title   = "Gauss-Newton convergence (period refinement)")
...
# ╔═╡ 8345281c-c96a-4d37-8fd7-c067012c960e
function rps_trig_ls(x0, T, N)
	sol = solve_replicator(Arps, x0, T)
	γ = 2π / T
	τs  = γ .* sol.t
    X   = hcat(sol.u...)'

	Φ = [cos.(j .* τs) for j = 0:N]
    Φ = hcat(Φ..., [sin.(j .* τs) for j = 1:N]...) 
    ...
    rms_error = norm(X - Φ * β) / sqrt(length(X))
	
    return c, d, rms_error
	
end

# ╔═╡ 743f68f0-e108-45f6-9694-2dbf2ba28c55
begin 
	c_1, d_1, rms_1 = rps_trig_ls([0.1; 0.2; 0.7], T_refined, 10)
	println("RMS error: $rms_1")
	println("c coefficients:\n", c_1)
	println("d coefficients:\n", d_1)
end 

# ╔═╡ 13115854-af85-4171-85af-38c79118b7ba
begin
    c, d, rms_error = rps_trig_ls([0.1; 0.2; 0.7], T_refined, 10)
    
    sol = solve_replicator(Arps, [0.1; 0.2; 0.7], T_refined)
    γ   = 2π / T_refined
    τs  = γ .* sol.t
    X   = hcat(sol.u...)'

    Φ = hcat([cos.(j .* τs) for j = 0:10]..., [sin.(j .* τs) for j = 1:10]...)
    β = Φ \ X
    X_fit = Φ * β

    plot(sol.t, X[:, 1], label="x1 ODE", color=:blue)
    plot!(sol.t, X[:, 2], label="x2 ODE", color=:red)
    plot!(sol.t, X[:, 3], label="x3 ODE", color=:green)
    plot!(sol.t, X_fit[:, 1], label="x1 fit", color=:blue,  linestyle=:dash)
    plot!(sol.t, X_fit[:, 2], label="x2 fit", color=:red,   linestyle=:dash)
    plot!(sol.t, X_fit[:, 3], label="x3 fit", color=:green, linestyle=:dash)
	plot!(xlabel="t", ylabel="xᵢ(t)", title="ODE vs trig fit (N=10), RMS=$(round(rms_error, sigdigits=2))")
end
...
# ╔═╡ baf11cab-1190-4cc2-a974-3835bc01942b
function FJFreplicator_cycle_ls(A, c, d, γ, m=100)
    n, N = size(d)
    F = zeros(m*n)
    J = zeros(m*n, n*(2N+1) + 1)
    col = 1
    for i = 1:m
        τ = 2π*i/m
        u = sum(c[:,j+1]*cos(j*τ) for j=0:N) +
...
# ╔═╡ 757dec8a-86fd-4f37-a96b-e7c4ac89b57a
begin
    u_test = [0.1; 0.2; 0.7]
    A_test = Arps
    eps = 1e-7
    
    Fu = replicator_rhs(u_test, A_test)
    J_an = Jreplicator(u_test, A_test)
    
    J_fz = zeros(3, 3)
    for q = 1:3
        u_p = copy(u_test)
        u_p[q] += eps
        J_fz[:, q] = (replicator_rhs(u_p, A_test) .- Fu) ./ eps
    end
...
# ╔═╡ 8b9fba88-c4c1-4f11-8cc3-a5ea86d72d7d
function build_constraints(n, N, d)
    nC = n * (N + 1)
    nD = n * N
    nV = nC + nD + 1
...
# ╔═╡ c038ba41-ca95-4a8f-96bb-7ec9b4415636

function gn_replicator_cycle(A, c, d, γ, m=100)
	max_iter=50
	tol=1e-10
	n, N  = size(d)
	B     = build_constraints(n, N, d)
	grad_norms = Float64[]
	for k = 1:max_iter
		
		F, J = FJFreplicator_cycle_ls(A, c, d, γ, m)
		g = J' * F
		...
		nCc = n * (N + 1)
		nDd = n * N
		c += reshape(p[1:nCc], n, N+1)
	    d += reshape(p[nCc+1:nCc+nDd], n, N)
	    γ += p[end]
		if k == max_iter
			println(norm(F))
		end
	...
	return c, d, γ, grad_norms
end

# ╔═╡ d4fee127-b1cc-4eb6-b2cd-b7db4980f67e
begin
    x0 = [0.1; 0.2; 0.7]
    T0 = rps_period_est(x0, 20.0)
    c0, d0, _  = rps_trig_ls(x0, T0, 20)
    γ0 = 2π / T0

    c_sol, d_sol, γ_sol, grad_norms = gn_replicator_cycle(Arps, c0, d0, γ0)
	println(grad_norms[end])
...
# ╔═╡ ea49fea6-12cf-4917-9841-f19e4df87094
begin
	ε = 1e-6
	x_0 = [0.1; 0.2; 0.7]
	A_testy = Arps
	T_0 = rps_period_est(x0, 20.0)
	c_0, d_0, rms_norm = rps_trig_ls(x_0, T_0, 20)
	γ_0 = 2π/T_0
	n, N = size(d_0)
	
	F0, J_real = FJFreplicator_cycle_ls(A_testy, c_0, d_0, γ_0)
	J_fd = zeros(size(J_real))

	for j = 0:N, k = 1:n
	    c2 = copy(c_0); c2[k, j+1] += ε
	    F2, _ = FJFreplicator_cycle_ls(A_testy, c2, d_0, γ_0)
	    col = j*n + k
	    J_fd[:, col] = (F2 - F0) / ε
	end

	for j = 1:N, k = 1:n
	    d2 = copy(d_0); d2[k, j] += ε
	    F2, _ = FJFreplicator_cycle_ls(A_testy, c_0, d2, γ_0)
	    col = n*(N+1) + (j-1)*n + k
	    J_fd[:, col] = (F2 - F0) / ε
	end
	
	F2, _ = FJFreplicator_cycle_ls(A_testy, c_0, d_0, γ_0 + ε)
	J_fd[:, end] = (F2 - F0) / ε
	
	println(norm(J_real - J_fd))
end
