 ...
 *		;
 *
 * The module exports four function:
 *
...
struct parser {
    enum { PARSER_NEUTRAL, PARSER_ERROR, PARSER_EOF } state;
...
static element_t parser_match(parser_t parser) {
    ...
    case TOKEN_STRING:
        ...
        elt->u.arg.string = parser->tokens[0]->string;
        parser->tokens[0]->string = nullptr;
        return elt;
    case TOKEN_LT:
        fd = 0;
        offset = 0;
    ...
    case TOKEN_CB_OPEN:
        ...
        fd = atoi(parser->tokens[1]->string);
    ...
    if (parser->tokens[offset + 1]->type == TOKEN_GT) {
        ...
        elt->u.redir_file.name = parser->tokens[offset + 2]->string;
        parser->tokens[offset + 2]->string = nullptr;
        return elt;
    }
    ...
    case TOKEN_STRING:
        ...
        elt = element_create(parser->tokens[offset]->type == TOKEN_LT
                                 ? ELEMENT_REDIR_FILE_IN
                                 : ELEMENT_REDIR_FILE_OUT);
        elt->u.redir_file.fd = fd;
        elt->u.redir_file.name = parser->tokens[offset + 1]->string;
        parser->tokens[offset + 1]->string = nullptr;
        return elt;
    case TOKEN_CB_OPEN:
        ...
        elt->u.redir_fd.fd2 = atoi(parser->tokens[offset + 2]->string);
        return elt;
    default:
