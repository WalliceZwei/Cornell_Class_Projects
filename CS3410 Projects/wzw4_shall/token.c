...
static token_t tokenizer_buffer(struct tokenizer *tokenizer, char c,
                                enum token_type tt);
...
static token_t tokenizer_string(struct tokenizer *tokenizer) {
    ...
    token->string = tokenizer->string;
    tokenizer->string = nullptr;
    tokenizer->strlen = 0;
...
token_t tokenizer_next(tokenizer_t tokenizer) {
    ...
    for (;;) {
        ...
        if (tokenizer->state == TOKENIZER_BUFFERED) {
            ...
            const int next_c = reader_next(tokenizer->reader);
            // was there an error with trying to read the next token?
            if (next_c == EOF) {
...
void tokenizer_free(tokenizer_t tokenizer) { free(tokenizer); }
