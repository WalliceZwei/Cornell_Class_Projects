...
def imread(imname, gray=False):
    if gray:
        img = np.array(Image.open(imname).convert("L")).astype(float) / 255.0
    else:
        img = np.array(Image.open(imname).convert("RGB")).astype(float) / 255.0
    return img
...
def gifwrite(imglist, imname):
    ...
    imglist[0].save(
        imname, save_all=True, append_images=imglist[1:], duration=100, loop=1
    )
