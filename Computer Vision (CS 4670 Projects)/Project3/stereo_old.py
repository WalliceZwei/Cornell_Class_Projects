...
def get_ncc_descriptors(img, patchsize):
    ...
    for x in range(len(img)):
        for y in range(len(img[0])):
            xbounds = (x - (patchsize // 2), x + (patchsize // 2))
            ybounds = (y - (patchsize // 2), y + (patchsize // 2))
            if xbounds[0] < 0 or xbounds[1] > len(img) - 1:
                pass
            elif ybounds[0] < 0 or ybounds[1] > len(img[0]) - 1:
                pass
            else:
                temp = img[
                    xbounds[0] : xbounds[1] + 1, ybounds[0] : ybounds[1] + 1, :
                ].copy()
...
def compute_ncc_vol(img_right, img_left, patchsize, dmax):
    ...
    ncc_vol = np.zeros((dmax, len(img_right), len(img_right[0])))
    for d in range(dmax):
        for i in range(len(img_right)):
            for j in range(len(img_right[0])):
