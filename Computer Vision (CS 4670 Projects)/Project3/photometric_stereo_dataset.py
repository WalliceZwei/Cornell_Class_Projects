...
def mountains(sz):
    ...
    for i in range(num):
        ...
        sigma = np.random.random_sample() * float(sz) / 5.0
...
def generate_dataset(name):
    ...
    albedo = (
        np.array(Image.open(os.path.join(dirname, "albedo.png")).convert("RGB")).astype(
            float
        )
        / 255.0
    )
...
def get_dataset(name, generate=True):
    ...
    images = [
        utils.imread(os.path.join(dirname, "images", "{:02d}.png".format(k)))
        for k in range(lights.shape[1])
    ]
