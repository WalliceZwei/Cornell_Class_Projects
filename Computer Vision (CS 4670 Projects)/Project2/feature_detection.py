...
def computeHarrisValues(srcImage):
    ...
    grad_x = ndimage.sobel(srcImage, axis=1, mode="nearest")
    grad_y = ndimage.sobel(srcImage, axis=0, mode="nearest")
    grad_xx = grad_x * grad_x
    grad_yy = grad_y * grad_y
    grad_xy = grad_x * grad_y
    sigma = 0.5
    grad_xx_smooth = ndimage.gaussian_filter(
        grad_xx, sigma, truncate=4.0, mode="nearest"
    )
    grad_yy_smooth = ndimage.gaussian_filter(
        grad_yy, sigma, truncate=4.0, mode="nearest"
    )
    grad_xy_smooth = ndimage.gaussian_filter(
        grad_xy, sigma, truncate=4.0, mode="nearest"
    )
    k = 0.1
    det_matrix = grad_xx_smooth * grad_yy_smooth - grad_xy_smooth * grad_xy_smooth
    trace_matrix = grad_xx_smooth + grad_yy_smooth
    harrisImage = det_matrix - k * (trace_matrix * trace_matrix)
    orientationImage = np.degrees(np.arctan2(grad_y, grad_x))
...
def computeLocalMaximaHelper(harrisImage):
    ...
    local_max = ndimage.maximum_filter(
        harrisImage, size=7, mode="constant", cval=-np.inf
    )
...
def detectCorners(harrisImage, orientationImage):
    ...
    local_maxima_image = computeLocalMaximaHelper(harrisImage)
    for row_index in range(height):
        for col_index in range(width):
            if local_maxima_image[row_index, col_index]:
                x_coord = col_index
                y_coord = row_index
                angle = orientationImage[row_index, col_index]
                detector_response = harrisImage[row_index, col_index]
                features.append((x_coord, y_coord, angle, detector_response))
...
def computeMOPSDescriptors(image, features):
    ...
    for i, f in enumerate(features):
        ...
        feat_x, feat_y, feat_angle, _ = f
        angle_rad = np.radians(feat_angle)
        T1 = get_trans_mx(np.array([-feat_x, -feat_y]))
        R = get_rot_mx(angle_rad)
        S = get_scale_mx(1 / 5, 1 / 5)
        T2 = get_trans_mx(np.array([windowSize / 2, windowSize / 2]))

        full_transform = T2 @ S @ R @ T1
        transMx = full_transform[:2, :]
        ...
        descriptor_vector = destImage.flatten()
        descriptor_mean = np.mean(descriptor_vector)
        descriptor_vector = descriptor_vector - descriptor_mean
        descriptor_variance = np.var(descriptor_vector)
        if descriptor_variance < 1e-10:
            descriptor_vector = np.zeros_like(descriptor_vector)
        else:
            descriptor_vector = descriptor_vector / np.sqrt(descriptor_variance)
        desc[i, :] = descriptor_vector
...
def produceMatches(desc_img1, desc_img2):
    ...
    dist_matrix = spatial.distance.cdist(desc_img1, desc_img2, "sqeuclidean")

    for idx_1 in range(dist_matrix.shape[0]):
        dist_vec = dist_matrix[idx_1]
        idx_2 = np.argmin(dist_vec)
        best_dist = dist_vec[idx_2]
        if len(dist_vec) > 1:
            temp = dist_vec.copy()
            temp[idx_2] = np.inf
            second_best_dist = np.min(temp)
            if best_dist < 1e-5 and second_best_dist < 1e-5:
                score = 1.0
            else:
                score = best_dist / second_best_dist
        else:
            score = 0.0  # 1 feature in image2
        matches.append((idx_1, idx_2, score))
