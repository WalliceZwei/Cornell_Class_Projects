...
def cross_correlation_2d(img, kernel):
    ...
    if len(kernel.shape) == 2:
        ...
        img = np.pad(img, ((m // 2, m // 2), (n // 2, n // 2), (0, 0)), mode="constant")
        if len(img.shape) == 2:
            for i in range(height):
                for j in range(width):
                    img[i, j] = np.sum(img[i : i + m, j : j + n] * kernel)
        elif len(img.shape) == 3 and img.shape[2] == 3:
            color = img.shape[2]
            for i in range(height):
                for j in range(width):
                    for k in range(color):
                        img[i, j, k] = np.sum(img[i : i + m, j : j + n, k] * kernel)
        else:
...
def gaussian_blur_kernel_2d(sigma, height, width):
    ...
    if height % 2 == 0 or width % 2 == 0:
        raise Exception("Invalid Kernel Dimensions")
    gbker = np.zeros(height, width)
    h_len = -1 * (height // 2)
    w_len = -1 * (width // 2)
    for x in range(height):
        for y in range(width):
            gbker[x, y] = np.exp(
                -((h_len + x) ** 2 + (w_len + y) ** 2) / (2 * sigma**2)
            ) / (2 * np.pi * (sigma**2))
    gbker /= gbker.sum()
    return gbker
    # TODO-BLOCK-END
